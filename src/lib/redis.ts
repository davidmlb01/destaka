...
// rateLimit:          fail-open, null quando Redis indisponivel (rotas baratas)
// rateLimitStrict:    fail-closed, lanca erro quando Redis indisponivel (rotas de custo LLM)
// =============================================================================
 ...
 * Retorna o valor atual ou null se Redis indisponivel (fail-open).
...
export async function rateLimitStrict(key: string, ttlSeconds: number): Promise<number> {
  const redis = getRedis()
  if (!redis) {
    throw new Error('Rate limiter indisponivel. Tente novamente em instantes.')
  }
