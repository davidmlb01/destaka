...
export async function generateReviewReply(
  ...
  const bioLine = bio ? `\nSobre o profissional: ${sanitizeForPrompt(bio, 300)}` : ''
  ...
  const message = await getAnthropic().messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Você é o(a) responsável por ${safeName}, ${safeSpecialty} brasileiro(a).${bioLine}
...
- Use APENAS o que está descrito neste prompt. Não invente serviços, diferenciais, tecnologias, certificações ou características do consultório que não foram informados.
- NUNCA mencione horários de funcionamento específicos.
- Para notas 1-2: não mencione o nome do estabelecimento, convide para contato por telefone
- Para notas 4-5: mencione brevemente o serviço apenas se o paciente citou na avaliação
