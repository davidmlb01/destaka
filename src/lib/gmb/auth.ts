...
export async function getValidGmbToken(userId: string): Promise<string> {
  ...
  if (!refreshRes.ok) {
    ...
    // Sinaliza token inválido no banco para exibir banner no dashboard
    // e envia email de reconexão (fire-and-forget, não bloqueia o fluxo)
    try {
      ...
      // Falha ao sinalizar: não bloqueia o erro principal
    }
  ...
  // Salva o novo token criptografado e limpa o flag de token inválido
  await serviceClient
    .from('users')
    .update({ google_access_token_enc: encrypt(newToken), gmb_token_invalid: false })
    .eq('id', userId)
